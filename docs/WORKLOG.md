...
|---|---|---|---|
| COD-175 | the one timer | `hooks/useFocusTimer.ts` + `shell/FocusTimer.tsx`: a Pomodoro-length countdown (5–30 min, one `<select>`) in the nav footer — Start / countdown / pause / stop — persisted under `prepforge:timer` so a reload keeps it; minutes credited to the day they ran on; a summary on stop or when time is up (this run, today, questions opened, rated; "Another N min"). An arrival prompt once per browser session, modal, no X / Esc / outside click: Start or Not now. Today's orient shows "learned today" in place of the notes count. `SessionTimer.tsx` and `useLibrarySession.ts` deleted — their progress-diff counting moved into the timer. Also: a visible Collapse / Expand button at the top of the nav (user: "the left nav, I'm unable to open it" — the app bar's trigger went with the bar). | 5 min run: nav shows `04:45`; pause froze at `04:30` for 2.2 s; resume ran on; stop credited 32,535 ms to `2026-09-08` and opened "Timer stopped · 00:33 this run". A run seeded to end in 3 s: "Time's up · 05:00 this run", 300,000 ms credited, `run: null`. Arrival dialog: no visible Close, `pf-timer-asked=1` after Start. Collapse button toggles `data-state` expanded → collapsed. `tsc` 0, build ok, 8/8. |
| COD-174 | hover with intent | `hooks/useHoverIntent.ts`: one timer, armed only by a `pointermove` (mouse) whose coordinates moved ≥ 2px from the last — a scroll, a reflow and Chrome's post-scroll re-dispatch all arrive at the same coordinates and arm nothing. Rows preview after 250 ms (pane follows, URL does not; click / j/k commit), free lenses after 400 ms, billed lenses still need a press. Row hover is `bg-surface1` + `text-text`, the selected row `bg-surface0 font-medium`. **Bug found:** the URL → `selectedId` effect was keyed on `selectedId` too, so it re-ran after every preview and snapped the pane back to the URL's question — the old hover-select had been dead since the first click. Keyed on the URL id only. | Hover A* row 900 ms: heading A*, URL still `?id=q002`. List scrolled 600px under the resting pointer: heading unchanged. Hover ELI5 1 s: ELI5 open. Hide list (reflow under the pointer): lens and question unchanged. `tsc` 0, build ok, 8/8. |
| COD-173 | no app bar | `AppBar.tsx`, `ui/breadcrumb.tsx`, `page/underAppBar.ts` and `--app-bar-h` deleted; `nav.VIEW_LABELS` with them (the breadcrumb was its one reader). Search is a nav item under the brand ("Search… Ctrl K", a magnifier on the rail); Settings is in the nav footer only. A phone gets a floating bottom-left trigger for the sheet (top-left sat on the sticky heading's first word). Sticky offsets are constants: list `top-2` / `h-[calc(100vh-1rem)]`, heading `top-0`, aside `1rem`. | Desktop 1440: no `header.glass`, Settings once, list starts at 187px (was 247). Phone 420: trigger at 16.5px from left and bottom, opens the sheet with Search, six routes, Shortcuts, Settings. `tsc` 0, build ok, 8/8. |
| COD-172 | one filter row | `FilterBand`: the nine topic chips and three difficulty chips are a Topic `<select>` and a Difficulty `<select>` beside the search box — one row with Clear filters, Recall and Hide list. Native selects: the platform popover, typeable, the width of the chosen word. `Chip` stays (FeedView uses it). | List starts at 247px (was 346 after COD-168, 440 before it). Topic → DSA: 60 rows, first "How can I eliminate duplicate values in a table?", detail follows, Clear filters appears. `tsc` 0, build ok, 8/8. |
| COD-170 | Library nits | Sticky question heading: the 1rem under its hairline is padding inside the opaque box, not margin — as margin it was a band the tab row scrolled through in fragments. List rows wrap to two lines (`line-clamp-2`) instead of one line cut off. The page eyebrow is gone (`Page.eyebrow` prop deleted): "Recall" was on screen three times — nav brand, breadcrumb, eyebrow. Titles: `capture.py` wrote `title:` with `json.dumps` (ASCII-escaped) and `ingest._frontmatter_title` only stripped the quotes, so 5,288 bank strings read `Linear Models — scikit-learn`; the reader now decodes JSON, the writer uses `ensure_ascii=False`, `repair_titles.py` decodes an existing bank in place (surrogate pairs joined). Found on the way: `quiz_synth.py` never imported `re` or `hashlib` since the COD-164 split, so `ingest()` had been broken; fixed. | `elementFromPoint` sweep bar-bottom → header-bottom at scrollY 420: only header nodes. Row heights 39.7 / 19.9 px, `-webkit-line-clamp: 2`. No `uppercase` "Recall" `<p>`. Rebuilt bank: 16,415 questions (same), 16,415 / 16,415 ids with answer files, 0 literal escapes. `test_ingest_split.py` all passed. My first in-place write truncated `generated.json` (a failed `write_text`); rebuilt with `ingest.ingest("deterministic")`, 8 min 11 s. |
| COD-169 | rate → next | `QuestionDetail`: after a grade the row carries a **Next question · j** button (`onNext`, the same step j/k use); in Recall mode the rating buttons carry "grading moves to the next question". Not built: centring Study's card — the empty right column is the 46rem reading measure, and centring breaks the one-left-gutter rule in `page/README.md`. | Got it on q002 → "Rated Got it — back in 1d." + Next; Next → q003, heading at 66.6px under the bar. Recall on, reveal, Got it → q004 without a press. `tsc` 0, build ok, 8/8. |

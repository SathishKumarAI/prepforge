...
ROW_KEYS = set(bank.INDEX_FIELDS) | {"origin", "tags", "snippet"}


def test_browse_rows_never_carry_the_answer():
    for params in ({}, {"q": "kafka"}, {"topic": "AI"}):
        rows = api_questions.questions_browse(**params)["questions"]
        for row in rows[:200]:
...
def test_empty_query_returns_the_whole_bank_count():
    res = api_questions.questions_browse()
    assert res["total"] == len(bank._load_questions())
...
def test_search_finds_answer_text_the_index_cannot():
    ...
    res = api_questions.questions_browse(q="kafka")
...
def test_every_typed_word_must_appear():
    ...
    both = api_questions.questions_browse(q="kafka partition")["total"]
    one = api_questions.questions_browse(q="kafka")["total"]
    assert both <= one, (both, one)
    nonsense = api_questions.questions_browse(q="kafka zzzzqqqxnotaword")
    assert nonsense["total"] == 0, nonsense["total"]


def test_title_matches_outrank_body_matches():
    rows = api_questions.questions_browse(q="kafka")["questions"]
...
def test_filters_narrow_and_topics_stay_whole():
    ...
    res = api_questions.questions_browse(topic="AI")
    assert res["total"] < len(bank._load_questions())
...
def test_no_snippet_when_nothing_was_searched():
    rows = api_questions.questions_browse()["questions"]
...
def test_pages_tile_the_result_exactly():
    ...
    """
    whole = api_questions.questions_browse(q="kafka", limit=10_000)
    ids = [r["id"] for r in whole["questions"]]
    assert len(ids) > 60, f"only {len(ids)} hits — need enough to page"

    walked, offset = [], 0
    while True:
        page = api_questions.questions_browse(q="kafka", limit=25, offset=offset)
...
def test_has_more_is_false_on_the_last_page():
    total = api_questions.questions_browse(q="kafka")["total"]
    last = api_questions.questions_browse(q="kafka", limit=25, offset=(total // 25) * 25)
    assert last["has_more"] is False, (total, last["offset"], len(last["questions"]))
    first = api_questions.questions_browse(q="kafka", limit=25, offset=0)
...
def test_later_pages_drop_the_whole_match_payload():
    ...
    first = api_questions.questions_browse(q="kafka", limit=25, offset=0)
    later = api_questions.questions_browse(q="kafka", limit=25, offset=25)
...
def test_offset_past_the_end_is_empty_not_an_error():
    page = api_questions.questions_browse(q="kafka", limit=25, offset=1_000_000)
    assert page["questions"] == []
    assert page["has_more"] is False


def test_negative_offset_is_clamped():
    a = api_questions.questions_browse(q="kafka", limit=5, offset=-50)
    b = api_questions.questions_browse(q="kafka", limit=5, offset=0)
...
def test_related_is_expanded_with_titles():
    ...
    got = api_questions.question(with_related["id"])
...
def test_a_question_says_which_lenses_are_on_disk():
    ...
    q = api_questions.question("q001")
    assert "star" in q.get("cached_modes", []), q.get("cached_modes")
    assert api_questions.question("no-such-question").get("error") == "not found"

...
ROW_KEYS = set(bank.INDEX_FIELDS) | {"has_answer", "has_quiz"}


def test_index_returns_its_projection_and_nothing_else():
    rows = api_questions.questions_index()["questions"]
...
def test_the_flags_are_booleans_not_the_content_they_describe():
    ...
    for row in api_questions.questions_index()["questions"][:200]:
...
def test_batch_expands_related_only_when_asked():
    ...
    """
    ids = [q["id"] for q in bank._load_questions() if q.get("related")][:5]
    assert ids, "no question in the bank has a related list — is the pipeline built?"

    plain = api_questions.questions_batch(ids=",".join(ids))["questions"]
    for q in plain:
        for entry in q.get("related") or []:
            assert set(entry) <= {"id", "score"}, f"batch expanded without being asked: {entry}"

    rich = api_questions.questions_batch(ids=",".join(ids), expand="related")["questions"]
...
def test_expansion_does_not_disturb_the_order_asked_for():
    ids = [q["id"] for q in bank._load_questions() if q.get("related")][:6]
    got = api_questions.questions_batch(ids=",".join(ids), expand="related")["questions"]
...
def test_a_related_id_that_no_longer_exists_is_dropped_not_blank():
    ...
    assert bank._with_related(ghost, by_id)["related"] == []
...
def test_batch_returns_whole_questions_in_the_order_asked_for():
    ...
    ids = [q["id"] for q in bank._load_questions()[:20]][::-1]
    got = api_questions.questions_batch(ids=",".join(ids))["questions"]
    assert [q["id"] for q in got] == ids
...
def test_batch_skips_ids_the_bank_no_longer_has():
    real = bank._load_questions()[0]["id"]
    res = api_questions.questions_batch(ids=f"{real},not-a-real-id")
...
def test_index_covers_the_whole_bank():
    index = api_questions.questions_index()
    assert index["count"] == len(bank._load_questions())

...
| The nav, search, the focus timer, focus mode | `frontend/src/components/shell/` via `components/README.md` |
| The Library's list, filters, detail, recall mode, hover preview | `frontend/src/components/library/README.md` |
...
unreported value; a Radix dialog left open disables every hotkey; chrome never hides itself on
scroll; nothing selects on `mouseenter` — hover previews go through `hooks/useHoverIntent`; one
control per job (no duplicate Settings, no chip rows where a select does).
